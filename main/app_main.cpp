...
extern "C" esp_err_t app_main()
{
    ...
    /* Initialize driver */
    app_driver_handle_t onoff_handle = app_driver_onoff_init();
    app_driver_handle_t button_handle = app_driver_onboard_button_init();
    ...
    /* Call for buttons */
    struct gpio_button button1;
    button1.GPIO_PIN_VALUE = BUTTON_1_PIN;
    err = create_button(&button1, node);
    ABORT_APP_ON_FAILURE(err == ESP_OK, ESP_LOGE(TAG, "Failed to create generic switch button 1"));

    struct gpio_button button2;
    button2.GPIO_PIN_VALUE = BUTTON_2_PIN;
    err = create_button(&button2, node);
    ABORT_APP_ON_FAILURE(err == ESP_OK, ESP_LOGE(TAG, "Failed to create generic switch button 2"));

    on_off_plugin_unit::config_t plugin_unit_config;
    plugin_unit_config.on_off.on_off = DEFAULT_POWER;
    plugin_unit_config.on_off.lighting.start_up_on_off = DEFAULT_POWER;
    endpoint_t *endpoint = on_off_plugin_unit::create(node, &plugin_unit_config, ENDPOINT_FLAG_NONE, onoff_handle);

    // endpoint handles can be used to add/modify clusters.
    ABORT_APP_ON_FAILURE(endpoint != nullptr, ESP_LOGE(TAG, "Failed to create OnOff Plugin endpoint"));

    onoff_endpoint_id = endpoint::get_id(endpoint);
    ESP_LOGI(TAG, "OnOff Plugin created with endpoint_id %d", onoff_endpoint_id);

    // Mark deferred persistence for some attributes that might be changed rapidly
    // cluster_t *onoff_cluster = cluster::get(endpoint, OnOff::Id);
    // attribute_t *current_onoff_attribute = attribute::get(onoff_cluster, OnOff::Attributes::OnOff::Id);
    // attribute::set_deferred_persistence(current_onoff_attribute);
